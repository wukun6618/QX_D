...
classlocal.Reflash_buy_list         = 1                 # 0：无需刷新stock_level1_lsit 1:需要重新刷新stock_level1_lsit
classlocal.ATRLength1               = ATR_LEN+ATR_LEN   # 图标bar线数量为10
classlocal.ATRLength2               = 4*ATR_LEN         # 图标Bar线数量为20
classlocal.MAength1                 = 184#99            # 图标bar线数量为
classlocal.MAength2                 = 215#144           # 图标Bar线数量为
...
#飞龙
classlocal.shenlong_en              = 0
classlocal.feilong                  = 0
classlocal.feilongsp                = 8888
classlocal.volume                   = 0
#以逸待劳
        ...
        for obj in obj_list:
           ...
           # local_hold.loc[code,'Price_BuyK']         = obj.m_dOpenPrice
            ...
            if Price_SellY_Flag :
                ...
                MLlength        = classlocal.M_HL #20
                length          = classlocal.ATRLength1 + MLlength #5 + 20
                h_data_init     = ContextInfo.get_market_data_ex(['close','high','open','low','volume'],\
                    check_list,period = classlocal.Period_Type,end_time=td,count=(length+10),\
                ...
                LowPrice        = 900000
                lowmin          = lows.min()
                MLhighs         = highs[-MLlength:]
                MLlows          = lows[-MLlength:]
                ML_HSLS         = MLhighs + MLlows
                ML_value        = np.array(ML_HSLS)
                ML_value        = np.mean(ML_value) / 2
                if lowmin > 0 :
                    close           = np.array(closes)
                    low             = np.array(lows)
                    high            = np.array(highs)
                    LowPrice        = low[-1]
                    classlocal_ATR  = calculate_ATR(high,low,close,length)*classlocal.TC_ATRratio
...
def shengling_checkout():
    ...
    if highmax in high[-length:-4]:
        QXDF = (highmax - lowmin) / lowmin >= classlocal.sldf
        if QXDF :
            righthand  = 1
    if (righthand):
        #最高到最低的跌幅
        ...
        THIRD_           = (close[-3] > open[-3])
        FOURTH_YIN       = (close[-4] < open[-4])
        FIFTH_YIN        = (close[-5] < open[-5])
        #倒数第6天是阴K
        SIX              = close[-6] < open[-6]
        if SIX :
            DF_64        = abs(open[-6] - close[-4]) / close[-4] >= classlocal.sldf
        elif (close[-6] < close[-7]):
            DF_64        = abs(close[-6] - close[-4]) / close[-4] >= classlocal.sldf
        else :
            DF_64        = 0
        SIX_YIN          = SIX
        YIN_YANG         = FIRST_YANG and SENCOD_YIN and FOURTH_YIN and FIFTH_YIN and SIX_YIN
        lowtj = low[-2]<low[-3] and low[-2]<low[-4] and low[-2]<low[-5] and low[-2]<low[-6]
        #小阳K
        if(THIRD_):
            szxzf        =  abs(close[-3] - open[-3]) / open[-3] < 0.018
        else :
            szxzf        =  abs(open[-3] - close[-3]) / close[-3] < 0.015
        #最后一个大阴线跌幅
        sencondyindf     = abs(open[-2] - close[-2]) / close[-2] >= classlocal.slsecondedf
        #下影线幅度
        sencondxydf      = abs(close[-2] - low[-2]) / low[-2] <= classlocal.szxfd

        fith_first_df   = abs(close[-5] - close[-2]) / close[-2] >= 0.01
        second_df       = abs(open[-2] - close[-2]) / close[-2] >= 0.01

        last_c          = close[-1] > close[-2]
        shenling        = YIN_YANG and lowtj and szxzf and sencondyindf and sencondxydf and last_c and fith_first_df and second_df

        #print(classlocal.Kindex_time)
        #print('YIN_YANG','lowtj','szxzf','sencondyindf','sencondxydf:\n',YIN_YANG,lowtj,szxzf,sencondyindf,sencondxydf)
        classlocal.shenling                = shenling
        classlocal.shenlingsp              = lowmin
...
def QXLCK_checkout():
    ...
    '''
    if highmax in high[-length:-4]:
        QXDF = (highmax - lowmin) / lowmin >= classlocal.qxdf
        if QXDF :
            righthand  = 1
    '''
    stop  = 0
...
def feilong_checkout():
    ...
    classlocal.feilong    =  0
    if (righthand):
        first       = close[-1] > open[-1] and close[-1] > compare_values_max(close[-2],open[-2])
        #小K线
        second      = abs(close[-2]-open[-2])/compare_values_min(close[-2],open[-2]) < 0.02
        #涨停板
        third       = close[-3] >= open[-3] and high[-3] == close[-3]
        thirdzf     = (close[-3] - open[-4]) / open[-4] > 0.095
        tiaokong    = (low[-1] > high[-3]) and (low[-2] > high[-3])
        volumest    = volume[-2] > volume[-3]
        #print(f'first:{first},second:{second},third:{third},thirdzf:{thirdzf},tiaokong:{tiaokong},volumest:{volumest}')
        classlocal.feilong    = first and second and third and thirdzf and tiaokong and volumest
        classlocal.feilongsp  = high[-3]
        classlocal.volume     = volumest
    else:
        classlocal.feilong    = 0
        classlocal.feilongsp  = 8888
        classlocal.volume     = 0
...
def model_process(ContextInfo,check_list):
   ...
   # h_data              = get_market_data_ex_modify(ContextInfo,check_list,period_t,endtime,length1)
    ...
    for code in check_list:
        ...
        openmin             = opens.min()
        #34日
        len                 = 10#classlocal.modul_length
        lowmin              = lows[-len:].min()
        highmax             = highs[-len:].max()
        ...
        if((closemin > 0) and (openmin > 0) and (lowmin > 0) and highmax):
            ...
            ma120_7              = np.mean(close[-classlocal.MAength1-1:-7])
            #昨日34日收盘价均值
            ma211                = np.mean(close[-classlocal.MAength2-1:-1])
            ma211_7              = np.mean(close[-classlocal.MAength2-1:-7])
            #如果13日均线上穿34日均线，且昨日收盘价位于13日均线之上
            righthand            =  0
            ...
            if classlocal.shenlong_en :
                feilong_checkout()
                feilong                      = classlocal.feilong
                feilongsp                    = classlocal.feilongsp
            ...
            if feilong:
                strfeilong                   = ":"+"神龙"
                G_df.loc[code,'Price_SellS'] = decimal_places_are_rounded(feilongsp,2)
                buy_atr                      = calculate_ATR(high,low,close,ART_length)
                G_df.loc[code,'ATR_BuyK']    = buy_atr
                G_df.loc[code,'Tradingday']  = td
                sp_price                     = G_df.loc[code,'Price_SellS']
                ...
                #print(f'G_df\n,{G_df}')
                #print('一箭双雕',code)
                send(td,code,str(decimal_places_are_rounded(feilongsp,2)),strfeilong)
            if yydl:
                stryydl                      = ":"+"以逸待劳"
